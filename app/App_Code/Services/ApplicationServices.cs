...
namespace MyCompany.Services
{
    ...
    public class ApplicationServicesBase
    {
                ...
                if (_defaultSettings == null)
                {
                    ...
                    if (_defaultSettings == null)
                    {
                        ...
                        if (_defaultSettings == null)
                        {
                            ...
                            EnsureJsonProperty(ui, "theme.accent", "DarkKnight");
        ...
        public virtual int AccessTokenDuration(MembershipUser user)
        {
            // 15 minutes
            return 15;
        }

        public virtual int RefreshTokenDuration(MembershipUser user)
        {
            // 60 minutes x 24 hours x 7  = 7 days
            return ((60 * 24)
                        * 7);
        }
        ...
        public virtual UserTicket CreateTicket(MembershipUser user, string refreshToken, string accessTokenDuration, string refreshTokenDuration)
        {
            ...
            if (!(string.IsNullOrEmpty(accessTokenDuration)))
            {
                jTimeout = TryGetJsonProperty(DefaultSettings, accessTokenDuration);
                if (jTimeout != null)
                    accessDuration = ((int)(jTimeout));
            }
            if (!(string.IsNullOrEmpty(refreshTokenDuration)))
            {
                jTimeout = TryGetJsonProperty(DefaultSettings, refreshTokenDuration);
                if (jTimeout != null)
                    refreshDuration = ((int)(jTimeout));
            }
            return CreateTicket(user, refreshToken, accessDuration, refreshDuration);
        }
    ...
    public class StylesheetGenerator
    {
        ...
        public override string ToString()
        {
            ...
            if (!(string.IsNullOrEmpty(iconsFont)))
                result = Regex.Replace(result, "MaterialIcons-\\w+.woff2", (("MaterialIcons-" + iconsFont)
                                + ".woff2"));
